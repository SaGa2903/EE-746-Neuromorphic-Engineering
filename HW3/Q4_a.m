...
weight = zeros(N,N);   % each cell is a single number
delay = zeros(N,N);    % each cell is a single number
fanout = cell(1,N);   % each cell is an array of 50 numbers sorted  in ascending order

exc_delay = randi([1, 20], 0.8*N, N); % delays in ms
delay(1:0.8*N,:) = exc_delay;  % putting delays in first 80% exc neurons
    ...
    for neuron = 1:N
        ...
        if ~isempty(pre_neuron{neuron})
            for pre = pre_neuron{neuron}
                arrival_time{pre,neuron} = spike_time{pre} + delay(pre,neuron);
                if ~isempty(spike_times{neuron})
                    arrival_times_temp = arrival_time{pre,neuron}(arrival_time{pre,neuron}<=spike_times{neuron}(end));
                    tjlast = arrival_times_temp(end);
                    tik = spike_times{neuron}(end);
                    weight(pre,neuron) = weight(pre,neuron)*(1+Aup*exp(-(tik-tjlast)/tau_l));
                    weight(neuron, fanout{neuron}) = weight(neuron,fanout{neuron})*(1+Adown*exp(-(tik+delay(neuron,fanout{neuron})-tjlast)/tau_l));
                end 
                spike_times_temp = spike_time{pre}(spike_time{pre}<=(t-delay(pre,neuron))) + delay(pre,neuron);
